...
class Secrets {
	...
	constructor(options = {}) {
		...
		this.region =
			options.region || process.env.AWS_REGION || DEFAULT_REGION;
		...
		this.secretsmanager = new clientSecretsManager.SecretsManager({ region: this.region });
	}
	...
	configSync() {
		...
		const result = child_process.spawnSync('node', [__dirname$1 + '/readline.mjs'], {
	...
	createSecret(options = {}) {
		...
		if (options.secretsBinary) {
			...
			if (
				Object.prototype.toString.call(options.secrets) ===
				'[object String]'
			) {
...
const configSync = function (options = {}) {
	const secrets = new Secrets(options);
...
const secretSync = function (options) {
