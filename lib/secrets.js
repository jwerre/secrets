...
class Secrets {
	 ...
	 * @param  {Number} [options.maxBuffer=3145728] Largest the entire config can be in bytes (default: 3 Mb)
	 */
	constructor(options = {}) {
		...
		this.maxBuffer = 1024 * 1024 * 3; // set max size is 3 Mb

		if (typeof options.maxBuffer === 'number' && options.maxBuffer > 0) {
	...
	configSync() {
		...
		const result = spawnSync('node', [__dirname + '/_read.js'], {
			input: JSON.stringify(args),
			maxBuffer: this.maxBuffer,
		});

		if (result.error) {
			// 1. Check for System/Buffer Errors
			if (result.error.code === 'ENOBUFS') {
				throw new Error('Max buffer it too small to retrieve all secrets.');
			} else {
				throw new Error(`Error retrieving secrets: ${result.error.message}`);
			}
		} else if (result.status !== 0) {
			// 2. Check for Script Execution Errors (Non-zero exit codes)
			throw new Error(`Script exited with code ${result.status}`, result.stderr.toString());
		}

		const res = JSON.parse(result.stdout.toString());
	...
	async config(options = {}) {
		...
		const list = await this.listSecrets();
		...
		const promises = list.map((item) => {
			return this.getSecret({ id: item.Name, raw: true });
		});

		const secrets = await Promise.all(promises);

		return this._parseSecrets(secrets);
	}
	...
	async getSecret(options = {}) {
		...
		try {
			...
			if (
				...
				this.retryAttempts <= Secrets.MAX_RETRY_ATTEMPTS
			) {
				this.retryAttempts++;
				console.error(
					new Error(
						`Rate limit exceeded. Retry attempt ${this.retryAttempts} of ${Secrets.MAX_RETRY_ATTEMPTS}.`
					)
				);
				return new Promise((resolve, reject) => {
					setTimeout(() => {
						this.getSecret(options).then(resolve).catch(reject);
					}, 1050);
				});
			} else {
				throw err;
	 ...
	 * @param  {Number} [options.maxBuffer=65536] Largest amount of data in Bytes the secret can be (default: 65536 Bytes).
	 * Max secrete size is 65,536 Bytes. See: https://docs.aws.amazon.com/secretsmanager/latest/userguide/reference_limits.html#quotas
			...
			try {
				...
				if (err.code === 'ThrottlingException') {
					res = await new Promise((resolve, reject) => {
						setTimeout(() => {
							this.secretsmanager.listSecrets(params).then(resolve).catch(reject);
						}, 1050);
					});
				} else {
					throw err;
	...
	_parseSecrets(list) {
		...
		for (let secret of list) {
			let name = secret.Name.replace(this.namespace + this.delimiter, '').replace(
				this.env + this.delimiter,
				''
			);
